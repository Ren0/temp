  ...
  webSocket.on('message', (message) => {
    ...
    if (action === 'JOIN') {
      if (games[gameId]) {
        ...
        webSockets[jsonMessage.playerId].send(JSON.stringify({ gameId, result: 'JOINED', game: games[gameId].game.ascii() }));
        webSockets[games[gameId].creator].send(JSON.stringify({ gameId, result: 'JOINED', game: games[gameId].game }));
      }
    }
  });
