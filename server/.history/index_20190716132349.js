...
webSocketServer.on('connection', (webSocket, request) => {
  ...
  webSocket.on('message', (message) => {
    ...
    if (action === 'CREATE') {
      ...
      const payload = { gameId, chess: games[gameId].chess, mutation: 'CREATED' };
    ...
    if (action === 'FETCH') {
      ...
      if (!games[gameId]) {
        ...
        games[gameId] = { creator: playerId, chess: new Chess() };
        const payload = { gameId, chess: games[gameId].chess, mutation: 'GAMECREATED' };
        send(playerId, payload);
      } else {
        console.log('fetching game: ', gameId);
        games[gameId] = { ...games[gameId], joiner: playerId };
        const payload = { gameId, chess: games[gameId].chess, mutation: 'FETCHED' };
        send(playerId, payload);
    ...
    if (action === 'MOVE') {
      ...
      const payload = { gameId, chess, mutation: 'MOVE' };
